...
# 시계열 데이터: 1949.01 ~ 1960.12 월별 항공기 이용 승객 수 기록
# 초기 80% 데이터를 학습 데이터로 이용하여 학습 시킨 후, 20% 데이터를 예측하게 하여 실제값과 비교함
...
def load_data(window_size):
    ...
    # 시계열 데이터를 이용한 RNN 기반 학습시에는 window_size 개념 사용
    X, y = [], []
    for i in range(len(raw_passengers) - window_size):
        cur_passenger = raw_passengers[i:i + window_size]
        target = raw_passengers[i + window_size]    # 다음 시점의 값을 target으로 하여 train
...
def build_rnn_model(window_size):
    model = Sequential()
    # 자연어 데이터가 아닌 경우에는 embedding layer를 사용하지 않으므로 시계열 데이터의 경우 input_shape를 작성해주어야 한다.
    model.add(layers.SimpleRNN(4, input_shape=(window_size, 1)))
    # window_size = 시점의 개수, 1 = 한 시점의 데이터에서의 feature 개수 (승객 수)
